document.addEventListener('DOMContentLoaded', () => {
    ...
    const initialize = () => {
        ...
        if (!storedPin) {
            ...
            setMode('login');
            // Small delay to let animations finish before popping up biometric
    ...
    const checkBiometric = async () => {
        ...
        if (isEnabled && window.PublicKeyCredential) {
            ...
            // 2. AUTO-TRIGGER: Try to open the prompt immediately
            try {
                // We check if we can autofill/silent check (optional, complex)
                // Or just trigger it:
                await triggerBiometricAuth();
            } catch (e) {
                // If auto-trigger fails (some browsers block it without a click), 
                // we just stay silent and let the user click the button.
                console.log("Auto-biometric blocked, waiting for user click.");
            }
        }
    };
    ...
    const triggerBiometricAuth = async () => {
        try {
            ...
            // This brings up the Browser/System Dialog
            ...
            // Ignore "User Cancelled" or "Not Allowed" errors to avoid spamming UI
            if (err.name !== 'NotAllowedError' && err.name !== 'AbortError') {
                showFeedback("Use PIN or Tap Fingerprint Icon", "error");
                shakeUI();
            }
        }
    };
